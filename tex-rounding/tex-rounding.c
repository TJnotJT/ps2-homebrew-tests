...
void make_texture()
{
	for (int i = 0; i < TEX_WIDTH; i++)
	{
		...
		{
			texture_data[(i * TEX_WIDTH + j) * 4 + 0] = i * TEX_WIDTH + j; // R
			texture_data[(i * TEX_WIDTH + j) * 4 + 1] = i * TEX_WIDTH + j; // G
			texture_data[(i * TEX_WIDTH + j) * 4 + 2] = i * TEX_WIDTH + j; // B
...
qword_t* draw_sprite(qword_t* q)
{
	...
	u32 x0 = WINDOW_X << 4, x1 = (WINDOW_X + 512) << 4;
	u32 u0 = 0 << 4, u1 = 512 << 4;
	u32 y0 = WINDOW_Y << 4, y1 = (WINDOW_Y + 512) << 4;
	u32 v0 = 0 << 4, v1 = 512 << 4;
	u32 z0 = 0, z1 = 0;
...
void save_image()
{
  ...
  sprintf(filename, "mass:%s", "tex-rounding.bmp");

  if (write_bmp_to_usb(filename, g_frame_data, 2 * TEX_WIDTH, 2 * TEX_HEIGHT, g_frame.psm, my_draw_clear_send) != 0)
  {
