...
#define REGIONS_X (FRAME_WIDTH / (2 * TEST_REGION_WIDTH))
#define REGIONS_Y (FRAME_HEIGHT / (2 * TEST_REGION_HEIGHT))
#define TEST_REGIONS (REGIONS_X * REGIONS_Y)
...
qword_t* my_draw_line(qword_t* q, int region)
{
	...
	const int x = WINDOW_X + 2 * TEST_REGION_WIDTH * (region / REGIONS_Y);
	const int y = WINDOW_Y + 2 * TEST_REGION_HEIGHT * (region % REGIONS_Y);
  
	const int mask_x = (TEST_REGION_WIDTH << 4) - 1;
	const int mask_y = (TEST_REGION_HEIGHT << 4) - 1;

  int X0 = (x << 4) | (rand() & mask_x);
  int X1 = (x << 4) | (rand() & mask_x);
	int Y0 = (y << 4) | (rand() & mask_y);
	int Y1 = (y << 4) | (rand() & mask_y);
  const int Z = 0;
...
int main(int argc, char *argv[])
{
  ...
  init_gs();
	...
	char filename[64];
	sprintf(filename, "mass:line_test_2.bmp");
	...
	if (write_bmp_to_usb(filename, g_frame_data, FRAME_WIDTH, FRAME_HEIGHT, g_frame.psm, my_draw_clear_send) != 0)
		...
		printf("Wrote hline data to USB\n");
