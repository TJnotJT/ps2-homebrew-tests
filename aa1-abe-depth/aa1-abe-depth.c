...
#define TEST_REGION_WIDTH 64
#define TEST_REGION_HEIGHT 64
...
{
	const int aa1 = (region >> 0) & 1;
	const int abe = (region >> 1) & 1;
	const int alpha = ((region >> 2) & 1) ? 128 : 64;

	const int region_x = region % TEST_REGIONS_X;
	const int region_y = region / TEST_REGIONS_X;
	const int x = 16 * (WINDOW_X + TEST_REGION_WIDTH * region_x);
	const int y = 16 * (WINDOW_Y + TEST_REGION_HEIGHT * region_y);

	const int x0 = (x + 16 * TEST_REGION_PAD) | 8;
	const int y0 = (y + 16 * TEST_REGION_PAD) | 8;
	const int x1 = (x + 16 * TEST_REGION_PAD) | 8;
	const int y1 = (y + 16 * (TEST_REGION_HEIGHT / 3 - 2 * TEST_REGION_PAD)) | 8;
	const int x2 = (x + 16 * (TEST_REGION_WIDTH - 2 * TEST_REGION_PAD)) | 8;
	const int y2 = (y + 16 * (TEST_REGION_HEIGHT / 3 - 2 * TEST_REGION_PAD)) | 8;
  ...
  PACK_GIFTAG(q, GS_SET_PRIM(GS_PRIM_TRIANGLE, 0, 0, 0, abe, aa1, 0, 0, 0), GIF_REG_PRIM);
  q++;
...
int main(int argc, char *argv[])
{
  ...
  init_gs();
	...
	char filename[64];
	sprintf(filename, "mass:aa1_abe_test_color.bmp");
	...
	sprintf(filename, "mass:aa1_abe_test_depth.bmp");
