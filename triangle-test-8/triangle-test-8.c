...
qword_t* my_draw_triangle(qword_t* q, int region)
{
	...
	const int x = 16 * (TEST_REGION_PAD + WINDOW_X + TEST_REGION_WIDTH * region_x);
	const int y = 16 * (TEST_REGION_PAD + WINDOW_Y + TEST_REGION_HEIGHT * region_y);

	const int x0 = x + rand() % (16 * (TEST_REGION_WIDTH - 2 * TEST_REGION_PAD));
	const int y0 = y + rand() % (16 * (TEST_REGION_HEIGHT - 2 * TEST_REGION_PAD));
	const int x1 = x + rand() % (16 * (TEST_REGION_WIDTH - 2 * TEST_REGION_PAD));
	const int y1 = y + rand() % (16 * (TEST_REGION_HEIGHT - 2 * TEST_REGION_PAD));
	const int x2 = x + rand() % (16 * (TEST_REGION_WIDTH - 2 * TEST_REGION_PAD));
	const int y2 = y + rand() % (16 * (TEST_REGION_HEIGHT - 2 * TEST_REGION_PAD));
...
int main(int argc, char *argv[])
{
  ...
  init_gs();
	...
	char filename[64];
	sprintf(filename, "mass:triangle_test_8%s.bmp", (USE_AA ? "_aa" : ""));
