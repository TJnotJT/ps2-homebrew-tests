...
void make_texture()
{
	for (int i = 0; i < TEX_SIZE; i++)
	{
		...
		{
			int addr = ROT ? (j * TEX_SIZE + i) * 4 : (i * TEX_SIZE + j) * 4;
			texture_data[addr + 0] = j & 0xFF; // R
			texture_data[addr + 1] = (j >> 8) & 0xFF; // G
			texture_data[addr + 2] = 0; // B
			texture_data[addr + 3] = 0xFF; // A
		}
	}
}
...
qword_t* m_draw_sprite(qword_t* q, int region)
{
	...
	u32 x0 = ((WINDOW_X + SHF) << 4) + OFFSET_FRAC;
	u32 x1 = ((WINDOW_X + SHF + (region + 1)) << 4) + OFFSET_FRAC;
	u32 y0 = (WINDOW_Y + (region % 256)) << 4;
	u32 y1 = (WINDOW_Y + (region % 256) + 1) << 4;
	u32 z0 = 0;
	u32 z1 = 0;
...
void save_image(int part)
{
  ...
  sprintf(filename, "mass:" PREFIX "-tw%d-tbw%d-off%d-frac%d-rev%d-rot%d-shf%d-%d.bmp",
		TW, TBW, OFFSET_U, OFFSET_FRAC, REV, ROT, SHF, part);
