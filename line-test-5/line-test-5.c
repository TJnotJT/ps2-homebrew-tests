...
#define TEST_REGIONS (VERTICAL_LINES ? (FRAME_WIDTH / (2 * TEST_REGION_SIZE)) : (FRAME_HEIGHT / (2 * TEST_REGION_SIZE)))
...
qword_t* my_draw_line(qword_t* q, int region)
{
	...
	int dx0 = (rand() % (TEST_REGION_SIZE * 16));
	int dy0 = (rand() % (TEST_REGION_SIZE * 16));
	int dx1 = (rand() % (TEST_REGION_SIZE * 16));
	int dy1 = (rand() % (TEST_REGION_SIZE * 16));

	int X0 = (x << 4) + dx0;
	int Y0 = (y << 4) + dy0;
	int X1 = ((x + (VERTICAL_LINES ? 0 : (FRAME_WIDTH - TEST_REGION_SIZE)))  << 4) + dx1;
	int Y1 = ((y + (VERTICAL_LINES ? (FRAME_HEIGHT - TEST_REGION_SIZE) : 0)) << 4) + dy1;
