...
#define TEST_REGIONS_X 32
#define TEST_REGIONS_Y 64
...
{
	static int colors[] = { 128, 255 };
	const int x = WINDOW_X + TEST_REGION_SIZE * (region % TEST_REGIONS_X);
	const int y = WINDOW_Y + TEST_REGION_SIZE * (region / TEST_REGIONS_X);

	int fx0 = region % TEST_REGIONS_X - 8;
	int fy0 = (region / TEST_REGIONS_X) % 16 - 8;
	int cx = colors[(region / 1024) % 2];
	int c0 = cx * ((region / 512) % 2);
	int c1 = cx - c0;
...
int render_test()
{
  ...
  // Set up the drawing environment and clear the frame buffer
	...
	for (int region = 0; region < TEST_REGIONS; region++)
	{
		...
		if (region % 256 == 255 || region == TEST_REGIONS - 1)
		{
			dma_channel_send_normal(DMA_CHANNEL_GIF, packet, q - packet, 0, 0);
			dma_channel_wait(DMA_CHANNEL_GIF, 0);
			q = packet;
...
int main(int argc, char *argv[])
{
  ...
  init_gs();
	...
	char filename[64];
	sprintf(filename, "mass:line_test_7%s%s%s.bmp", VERTICAL ? "_v" : "_h", USE_AA ? "_aa" : "", USE_TEX ? "_tex" : "");
