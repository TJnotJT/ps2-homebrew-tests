...
qword_t* my_draw_point(qword_t* q, int region)
{
	int region_x = region / TEST_REGIONS_Y;
	int region_y = region % TEST_REGIONS_Y;

	const int x = WINDOW_X + TEST_REGION_SIZE * region_x;
	const int y = WINDOW_Y + TEST_REGION_SIZE * region_y;
...
int main(int argc, char *argv[])
{
  ...
  init_gs();
	...
	char filename[64];
	sprintf(filename, "mass:point_test.bmp");
